...
변수의 수가 m인 데이터에서 현 단계에 K 개의 군집이 있고, 각 군집에 n.i 개의 데이터가 있다면
각 군집의 SSE.i 와 전체 군집의 SSE는 다음과 같다.

@
SSE_ i = \sum \limits_ {j=1}^{n_ i} \sum \limits_ {k=1}^{m} (x_ {ijk} - \bar x_ {ik})^2 \\\ 
SSSE = \sum \limits_ {K}^{i=1} SSE_ i = \sum \limits_ {K}^{i=1} \sum \limits_ {j=1}^{n_ i} \sum \limits_ {k=1}^{m} (x_ {ijk} - \bar x_ {ik})^2
@
...
크기가 각각 $n_ 1, n_ 2$인 두 군집 $G_ 1, G_ 2$을 묶을 때 생기는 SSE의 증가분을 $d(G_ 1, G_ 2)$라고 표기하며, 다음과 같이 계산한다
...
[UPGMA vs WPGMA](https://www.mun.ca/biology/scarr/UPGMA_vs_WPGMA.htm)

가중치 적용 및 비가중치 적용 용어는 최종 결과를 나타내지 계산 과정을 나타내는 것은 아니다.     
