...
@
\begin{align\*}
1) & \lambda_{coord} \sum\limits_ {i=0}^{S^2}\sum\limits_ {j=0}^{B} \mathbb{1}_ {ij}^{obj} \[ (x_ i - \hat{x}_ i)^2 + (y_ i - \hat{y}_ i)^2 \] \\\ 
2) &+ \lambda_{coord} \sum\limits_ {i=0}^{S^2}\sum\limits_ {j=0}^{B} \mathbb{I}_ {ij}^{obj} \[ (\sqrt{w_ i} - \sqrt{\hat{w}_ i} )^2 + (\sqrt{h_ i} - \sqrt{\hat{h}_ i})^2 \] \\\ 
3) &+ \sum\limits_ {i=0}^{S^2}\sum\limits_ {j=0}^{B} \mathbb{I}_ {ij}^{obj} (C_ i - \hat{C}_ i)^2 \\\ 
4) &+ \lambda_{noobj} \sum\limits_ {i=0}^{S^2}\sum\limits_ {j=0}^{B} \mathbb{I}_ {ij}^{noobj} (C_ i - \hat{C}_ i)^2 \\\ 
5) &+ \sum\limits_ {i=0}^{S^2} \mathbb{I}_ {i}^{obj} \sum_ {c \in \text{classes} } (p_ i (c) - \hat{p}_ i (c))^2
\end{align\*}
@
