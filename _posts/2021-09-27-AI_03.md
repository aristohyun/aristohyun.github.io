...
> 에 있는 모든 정형식을 참(T)으로 만드는 모델이, 를 참(T)으로 만든다

 는 를 논리적으로 귀결한다(logically entail)
= 는 를 논리적으로 따른다(logically follow)
= 는 의 논리적 결론(logical consequence)이다

델타 = 오메가
P, P->Q = Q
P와 P->Q가 참이면, Q는 참이 된다
참으로 알려진 델타로 부터 알려지지 않은 참인 오메가를 찾는 것을 추론이라고 한다
...
> 참인 논리식들이 논리적으로 귀결하는 새로운 논리식을 만들어내는 기계쩍으로 적용되는 규칙
...
![image](https://user-images.githubusercontent.com/32366711/135025623-e5fdb0ae-fbc2-4df5-bcc7-38d67be1ed34.png)

이게 참 이라면, 
R이 참인지 확인하려고 한다, 그럼 R 대신 ~R을 넣었을 때 ㅁ순이 생기는지 본다
