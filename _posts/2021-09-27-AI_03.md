...
> <red>참인 논리식</red>들이 논리적으로 귀결하는 새로운 논리식을 만들어내는 기계적으로 적용되는 규칙
...
이게 참 이라면, 
R이 참인지 확인하려고 한다, 그럼 R 대신 ~R을 넣었을 때 모순이 생기는지 본다
...
[^resolution]: 두개의 논리합절이 같은 기호의 긍정과 부정 리터럴를 서로 포함하고 있을 때,     
해당 리터럴들을 제외한 나머지 리터럴들의 논리합절 생성 $ p \vee q, \neg p \vee r \vdash q \vee r $
