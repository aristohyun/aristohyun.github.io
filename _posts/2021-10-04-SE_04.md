...
DevOps는 에자일의 특징을 근간으로함. 에자일은 기민함. 빠른것
특히 더 중요한 것은 신속하게 개발해서 빨리 개발하는것에 초점을 맞추는것이아님.
오퍼레이션에있음
소프트웨어시스템이 운영되는 중에도 프로젝트의 일부다. 소프트웨어가 폐기될때까지

오퍼레이션과정이라는것도 팀이 협업을해서 소프트웨어를 운영해야한다
애자일리티, 협업(개발팀+운영팀), 자동화(신속하게개발하기위함) 
전통적인 소프트웨어개발이 어떤문제가 있었냐
Silo기반 접근 방법
Silo : 한쪽을 다 소진하면 그다음 창고를 개방
폭포수모델을 근ㄱㄴ으로했던거는 한쪽, 분석을 다 끝내야 다음 단계를 진행
이런식으로 하다보면 od기는 문제가 각 작업들을 독립적으로 진행함
분석팀 개발팀 운영팀 등등
한쪽에서 딜레이가 생기면 그 이후도 계속 딜레이가 생김
...
plan : 내가 현재 해야할일을 잘게 나눔
이걸 얼마만큼의 주기로 할것인가
build : 컴파일과 유사, 합쳐서 소프트웨어시스템 생성. 
test
release : 
deploy : 
패키징, 딜리버리
실제 운용
다음이 서포트해야 데브옵스가 작동할수있음
CICD
이게 가능해지려면 소프트에ㅜ어가 어떤 구조를가져야하는가
Micro아키텍쳐 
인프라를 구성하는 요소들이 굉장히 많음. 이걸 일일이 셋팅하기 어려움. 이것마저 코드화
코드를 실행시키면 인프라가 자동셋팅

툴체인의 도구들이 따로따로 논다면, 도구를 사용하는데의미가적음.
호환성이 중요
실패하는이유 – 관행을 버리지못함
...
R&D smallsize + Agile
코드와 문서가 달라질수있음
코드로부터 설계정보를 뽑아냄  유사한 다른 도메인에 적용할 수 있는 정보

연구개발 R&D( DSDM)


요구사항정의서 다음 project management plan
프로젝트괸리계획서
요구사항정의서
소프트웨어 설계서
애자일 데브옵스엣는 문서작업을 강요하진않음
매읾코드가 바뀌는데 그때마다 업데이트할수는없으니
