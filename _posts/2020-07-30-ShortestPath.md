...
#### 핵심 알고리즘
<span class="margin"> 알고리즘의 매 단계에서 집합 S 안에 있지 않은 정점 중에서 가장 distance 값이 작은 정점을 S에 추가한다. </span>    
<span class="margin"> 새로운 정점 u가 S에 추가되면, S에 있지 않은 다른 정점들의 distance 값을 수정한다. </span>    
<span class="margin"> 시작 기준점이 u로 바뀌었기 때문에, 새로 추가된 정점 u를 거쳐서 정점까지 가는 거리와 기존의 거리를 비교한다.  </span>    
<span class="margin"> 그 후 더 작은 거리값을 기준으로 distance값을 수정한다. 아래와 같은 수식을 이용하면 될 것이다. </span>    
