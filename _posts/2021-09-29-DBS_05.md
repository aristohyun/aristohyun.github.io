...
|종류| 순수 관계 연산자 <br/> 일반집합 연산자 | 튜플 관계 해석 <br/> 도메인 관계 해석 |{: .aligncenter}
...
\pi_ {NAME, SALARY}(\sigma_ {DNO=5}(EMPLOYEE)) \\\ 
 \\\ 
DNO5_EMPS \leftarrow \sigma _ {DNO = 5}(EMPLOYEE)
RESULT \leftarrow \pi _ {NAME, SALARY}(ENO5_EMPS)
$
...
@
1) \rho_ S (R)
2) \rho_ {(B_ 1, \cdots, B_ n)}(R)
3) \rho_ {S(B_ 1, \cdots, B_ n)})(R)
@
